...
class KeySchedule:
    ...
    def pc_1(key):
        ...
        for i in range(7):
            for j in range(8):
                permutedStr += key[KeySchedule.PC_1[i][j] - 1]
    ...
    def pc_2(inputString):
        ...
        for i in range(6):
            for j in range(8):
                permutedStr += inputString[KeySchedule.PC_2[i][j] - 1]
    ...
    def generateSubkeysForDecryption(key):
        ...
        for rounds in range(1, 16 + 1):
            if rounds == 1:
                pass
            elif rounds in [2, 9, 16]:
                # We shift right by 1 bit
                D = KeySchedule.shiftRight(D, 1)
                C = KeySchedule.shiftRight(C, 1)
            else:
                # We shift right by 2 bits
                D = KeySchedule.shiftRight(D, 2)
                C = KeySchedule.shiftRight(C, 2)

            wholeKey = C + D
            subkeys[rounds - 1] = KeySchedule.pc_2(wholeKey)

        # print("SUBKEY: " + str(subkeys))
        return subkeys